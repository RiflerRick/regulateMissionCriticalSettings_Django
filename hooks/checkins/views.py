...
RETURN_MSG = settings.RETURN_MSG
DIR_ITEM_SET = set(settings.DIRS)
OWNER_FILE_NAME = settings.OWNER_FILE_NAME
INTIMATE_OWNERS = settings.INTIMATE_OWNERS
CHECK_IN_FILES = settings.CHECK_IN_FILES
WEBHOOK_SECRET = settings.WEBHOOK_SECRET
REPO_NAMES = settings.REPO_NAMES
...
def index(request):
    """
    index page for the purpose handling get requests to this page
...
def attempt_to_get_owner_file(repo_login, repo_name, filepath, branch):
    """
    attempt to find the CODEOWNERS file from the top level directory.
    ...
    url = os.path.join("https://api.github.com", "repos", repo_login, repo_name, "contents",
                       owner_filepath)
    url += parameters
    r = requests.get(url)
    response = r.json()
    try:
        ...
        for line in decoded_content:
            ...
            if re.match(filepath_regex, filepath):
                # the above line is buggy, wrt the regex used in github
...
def get_position_to_comment(file):
    ...
    :param file: file as a dictionary
    :return: line number
    """
    patch = file["patch"]
    patch = patch.split('@@')
...
def process_payload(thread_name, parsed_json):
    ...
    logging_file.write(str(json.dumps(parsed_json, indent=4, separators=(',', ':'))))
    ...
    try:
        ...
        if pr_head_branch not in BRANCHES:
            return RETURN_MSG
        ...
        for commit in commits:
            ...
            for file in files:
                ...
                if CHECK_IN_FILES:
                    # checks inside files this time and not folders
                ...
                elif dir_items.intersection(DIR_ITEM_SET):
                    ...
                    if INTIMATE_OWNERS == True and dir_items not in owners_in_dir:
                        ...
                        r = attempt_to_get_owner_file(repo_login, repo_name, filepath,
                                                      pr_base_branch)
                        if r == -1:
                            # it means the filepath has only one file or the owners file
                            # does not exist
                            pass
                        else:
                            owners = r
...
def authenticate(body, hash):
    """
    authenticate the webhook so that we know it is coming from the correct source

    :param body: body of the payload
    :param hash: secret message from headers of the request
    :return:
    """
